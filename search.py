...
def calc_vector_space(query,id_list):
	...
	for term in term_list:
		if term in t_inverted_index:
			q_vector.append((1+log10(query.count(term)))*log10(doc_N/float(len(t_inverted_index[term]))))

	q_vector = np.nan_to_num(np.array(q_vector))
	for ids in vector_list:
		q_doc_cos_list[ids] = np.dot(q_vector,vector_list[ids])/(np.linalg.norm(q_vector,ord=2)*(np.linalg.norm(vector_list[ids],ord=2)))
		...
		if share_count == 0:
			share_count = 1
		q_doc_cos_list[ids] = np.nan_to_num(q_doc_cos_list[ids])*log10(sqrt(play_count))*log10(share_count)
